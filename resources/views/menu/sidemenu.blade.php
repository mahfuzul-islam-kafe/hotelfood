...
<div id="menu-head" class="row pmclr m-0 p-0">
  ...
  <div class="col-md-9 p-lg-3 p-md-2">
    <div class="row">
      <div class="col-md-6">
        <h5 style="color: #E5D5BF; font-size: 24px;">@if(session('method', 'default') == 'delivery') {{session('address')}} @else A EMPORTER @endif</h5>
...
<div id="menu-head2" class="row pmclr m-0 p-0">
  ...
  <div class="col-12 p-lg-3 p-md-2" style="background-color: #000;">
    <div class="row">
      <div class="col-6 p-2">
        <h5 style="color: #E5D5BF; font-size: 14px;">@if(session('method', 'default') == 'delivery') {{session('address')}} @else A EMPORTER @endif</h5>
...
<div class="col p-md-9 content-menu">
  @yield('menubody')
  @endsection
  ...
  <script>
    function handleScrollsideMenu() {
      var navbar2 = document.getElementById("menu-side2");
      var scrollThreshold = 164;
      var currentScrollPos = window.pageYOffset || document.documentElement.scrollTop;
      if (currentScrollPos > scrollThreshold) {
        navbar2.style.top = "57px";
      } else {
        navbar2.style.top = (scrollThreshold - currentScrollPos) + "px";
      }
    }
    window.onscroll = handleScrollsideMenu;
  ...
  <div class="modal fade" id="exampleModaltimeslot" tabindex="-1" aria-labelledby="exampleModaltimeslot" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="row">
    @php
        $noonSlotsAvailable = count($timeSlots['noon']) > 0 && collect($timeSlots['noon'])->filter(function($slot) use ($currentDateTime) {
            $startDateTime = \Carbon\Carbon::parse($currentDateTime->toDateString() . ' ' . explode(' - ', $slot)[0]);
            return $currentDateTime->lt($startDateTime) && $startDateTime->diffInMinutes($currentDateTime) > 30 && $startDateTime->isSameDay($currentDateTime);
        })->count() > 0;

        $eveningSlotsAvailable = count($timeSlots['evening']) > 0 && collect($timeSlots['evening'])->filter(function($slot) use ($currentDateTime) {
            $startDateTime = \Carbon\Carbon::parse($currentDateTime->toDateString() . ' ' . explode(' - ', $slot)[0]);
            return $currentDateTime->lt($startDateTime) && $startDateTime->diffInMinutes($currentDateTime) > 30 && $startDateTime->isSameDay($currentDateTime);
        })->count() > 0;
    @endphp

    @if ($noonSlotsAvailable || $eveningSlotsAvailable)
        @if ($noonSlotsAvailable)
